...
```{r, message =F}
setwd("~/Documents/Projects/myeloid/haemonc_smalltasks/bin/Haemonc_requests/EBH-929")
...
for(sample in 1:length(total_files)) {
  ...
  vcf_1 = readr::read_delim(paste("data_vcf/", sample_folder, "/002_norm.vcf" , sep = ""), comment = '#', col_names = F, delim = "\t")
  ...
  vcf_3 = readr::read_delim(paste("data_vcf/",sample_folder, "/003_norm.vcf" , sep = ""), comment = '#', col_names = F, delim = "\t")
...
for(sample in total_files) {
  ...
  # print(p2)
  fit <- lm(vcf_all$Old_capture ~ vcf_all$New_capture, data = vcf_all)
  q = plot_ly(vcf_all, x = vcf_all$Old_capture, y = vcf_all$New_capture, text = paste("Site: ", vcf_all$Site),
        type = "scatter",mode = "markers") 
  print(q)
  htmlwidgets::saveWidget(as_widget(q), paste("data_vcf_plots/", sample, ".html",sep =""))
